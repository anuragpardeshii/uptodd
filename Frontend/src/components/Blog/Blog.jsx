...
const handleCheckboxChange=(e)=>{
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        const response = await axios.get(`https://ten-uptodd.onrender.com/api/comments/${encodeURIComponent(title)}`);
...
const latestComment=comments[comments.length - 1] //Get the latest comment if available
  ...
  const handleSubmit=(e)=>{
    ...
    axios.post(`https://ten-uptodd.onrender.com/api/comments/${encodeURIComponent(title)}`,newComment)
    .then(response=>{
...
const handleDelete=async(commentId)=>{
  try {
    await axios.delete(`https://ten-uptodd.onrender.com/api/comments/${encodeURIComponent(title)}/${commentId}`);
...
//if approved, proceed with submitting the reply
  try {
    ...
    // Only write this logic if you want to display reply text
     const response=await axios.post(`https://ten-uptodd.onrender.com/api/comments/${encodeURIComponent(title)}/${commentId}/reply`,{
      author:replyAuthor,
      email:replyEmail,
      text:replyText,
     });
...
const handleDeleteReply=async(commentId,replyId)=>{
  try {
    const response= await axios.delete(`https://ten-uptodd.onrender.com/api/comments/${encodeURIComponent(title)}/${encodeURIComponent(commentId)}/reply/${replyId}`);
